...
def prioritize_single(row_index):
    ...
    try:
        ...
        def get_safe(key, default="N/A"):
            return row.get(key, default) if pd.notna(row.get(key)) else default
...
2. After the table, include a **4–5 sentence concluding paragraph** (under the Markdown heading **## Conclusion**) that summarizes the key takeaways and recommends next steps (e.g., pilot testing, data validation, stakeholder alignment).
